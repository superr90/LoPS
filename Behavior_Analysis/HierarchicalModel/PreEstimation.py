...
def _readAuxiliaryData(data_time):
    '''
...
def preEstimation(filename_list, save_base):
    ...
    data_time = "before"
    adjacent_data, locs_df, adjacent_path, reward_amount = _readAuxiliaryData(data_time)
...
def CalculateUtility(filename, saveFolder):
    ...
    x = "{}/{}-with_Q-{}-{}.pkl".format(saveFolder, filename.split("/")[-1].split(".")[0],
                                        'human' if is_human == 1 else 'monkey',
                                        '7str' if is_7str == 1 else '6str')
    x = x.split("/")[-1]

    exists_name = os.listdir("../../monkey_data_before/monkeyUtilityData/data2018/")
    if x in exists_name:
        return
...
if __name__ == '__main__':
    print("AA" * 20)
    data_time = "before"
    path = "../../monkey_data_before/monkeyCorrectedTileData/data2018/"
    fileFolder = "../../monkey_data_before/monkeyCorrectedTileData/data2018/"
    saveFolder = "../../monkey_data_before/monkeyUtilityData/data2018/"
    fileNames = os.listdir(fileFolder)
    filePaths = [fileFolder + f for f in fileNames]

    exists = []
    tempPaths = [saveFolder + f[:-7] + "-with_Q-monkey-7str.pkl" for f in fileNames]
    ...
    #     print(i,"="*100,i)
    #     CalculateUtility(filePaths[i], saveFolder)
    with multiprocessing.Pool(processes=10) as pool:
        result = pool.map(
            partial(CalculateUtility, saveFolder=saveFolder), filePaths)

        # omegaL-04-Mar-2021-1pFlip.pickle
        # omegaL-18-Dec-2020-1pFlip.pickle
        # omegaL-28-Nov-2020-1pFlip.pickle
        # Patamon-15-May-2021-1pFlip.pickle
        # Patamon-17-May-2021-1pFlip.pickle
        # Patamon-18-Jun-2021-1pFlip.pickle
        # Patamon-20-May-2021-1pFlip.pickle
        # Patamon-27-Apr-2021-1pFlip.pickle
