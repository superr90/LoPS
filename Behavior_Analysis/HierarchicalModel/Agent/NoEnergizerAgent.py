...
class NoEnerTree:
    def __init__(self, adjacent_data, locs_df, reward_amount, root, energizer_data, bean_data, ghost_data, reward_type,
        ...
        :param bean_data: A list of positions of beans. Each position should be a 2-tuple.
        ...
        for each in ["left", "right", "up", "down"]:
            ...
            if None == self.current_node.parent and isinstance(tmp_data[each], float):
                ...
                if ignore:
                    ...
                    existing_beans, existing_fruit, existing_energizers,ghost_status = self._computeReward(cur_pos)
                    # Compute risk
                    ...
                    if cur_pos in [each.name for each in self.current_node.path]:
                        ...
                        exact_risk, potential_risk, existing_energizers, ghost_status = self._computeRisk(cur_pos)
    ...
    def _computeReward(self, cur_position):
        ...
        # Fruit reward
        if not isinstance(existing_fruit, float) and None != existing_fruit and not isinstance(self.reward_type,
                                                                                               float) and None != self.reward_type:
        ...
        ghost_potential_reward = 0.0
        fruit_potential_reward = 0.0
        return exact_reward, ghost_potential_reward, fruit_potential_reward, existing_beans, existing_fruit, existing_energizers,ghost_status
    ...
    def _computeRisk(self, cur_position):
        ...
        if isinstance(existing_energizers, float) or cur_position not in existing_energizers:
            ...
            # Risk for eating the energizer
            exact_risk -= self.reward_amount[2]
            existing_energizers.remove(cur_position)
            ghost_status = [4 if each != 3 else 3 for each in
                            ghost_status]  # change ghost status (if energizer is eaten, ghost status will be changed to scared)
        else:
            pass
        if ifscared1 <= 2 or ifscared2 <= 2:  # ghosts are normal; use "or" for dealing with dead ghosts
            if 3 == ifscared1:
                ...
                if cur_position == self.ghost_data[1]:
                    self.is_eaten = True
                    return exact_risk, potential_risk, existing_energizers, ghost_status
            elif 3 == ifscared2:
                # Pacman is eaten
                if cur_position == self.ghost_data[0]:
                    self.is_eaten = True
