...
class EvadeTree:
    ...
    def __init__(self, adjacent_data, locs_df, reward_amount, root, energizer_data, bean_data, ghost_data, reward_type, fruit_pos, ghost_status, last_dir, ghost_name,
        ...
        elif self.ghost_name == "ghost3":
            self.ghost_data = [self.ghost_data[2]]
            self.ghost_status = [self.ghost_status[2]]
        elif self.ghost_name == "ghost4":
            self.ghost_data = [self.ghost_data[3]]
            self.ghost_status = [self.ghost_status[3]]
        else:
    ...
    def _computeRisk(self, cur_position):
        ...
        ifscared = ghost_status[0] if not isinstance(ghost_status[0], float) else 0 # ghost1
        exact_risk = 0.0
        potential_risk = 0.0
        if ifscared == 2 or ifscared == 1:  # ghosts are normal; use "or" for dealing with dead ghosts
            # Pacman is eaten
            if cur_position == self.ghost_data[0]:
...
#     print("Evade(Blinky) Choice : ", choice, Q)
# 
# # Evade blinky agent
#     agent = EvadeTree(
#         adjacent_data,
...
#     _, Q = agent.nextDir(return_Q=True)
#     choice = agent.dir_list[makeChoice(Q)]
#     print("Evade(Clyde) Choice : ", choice, Q)