...
def Parsing(filePath, sets, saveFolder, needSipGram):
    ...
    Dict = {
        0: "G", 1: "L", 2: "1", 3: "2", 4: "A", 5: "E", 6: "N", 7: "V", 8: "S"
    }
    ...
    savePath = saveFolder + "-".join(filePath.split("/")[-1].split("-")[:4]) + "-" + \
               filePath.split("/")[-1].split("-")[4][0] + ".pkl"
    newData.to_pickle(savePath)


def ParsingMain(Monkey, date):
    result = pd.read_pickle("../MonkeyData/GrammarFinall/" + date + "/" + Monkey + ".pkl")
    fileFolder = "../MonkeyData/CorrectedWeightData/" + date + "/"
    fileNames = os.listdir(fileFolder)
    filePaths = [fileFolder + fileName for fileName in fileNames if Monkey.lower() in fileName.lower()]

    saveFolder = "../MonkeyData/GrammarData/" + date + "/"
    # for i in range(2):
    #     Parsing(filePaths[i], sets=result["sets"], saveFolder=saveFolder, needSipGram=result["skipGram"])

    with multiprocessing.Pool(processes=12) as pool:
        pool.map(partial(Parsing, sets=result["sets"], saveFolder=saveFolder, needSipGram=False), filePaths)


def LoPSComplexity(filePath, sets, skipGram):
    """
    Average complexity per round
    """
    print(filePath)
    if "N" not in sets:
        sets.append("N")
    ...
    for idx, grp in data.groupby("game"):
        ...
        meanDepth3 = np.mean(depth)
        allDepth += depth
        meanDepthPerGame.append(meanDepth3)

    return meanDepthPerGame, allDepth


def LoPSC_omplexity_monkey(Monkey, date):
    gramPath = "../MonkeyData/GrammarFinall/" + date + "/" + Monkey + ".pkl"
    result = pd.read_pickle(gramPath)
    ...
    # resultDepth = []
    # for i in range(2):
    #     r = LoPSComplexity(filePaths[0], sets=result["sets"], skipGram=result["skipGram"])
    #     resultDepth.append(r)
    with multiprocessing.Pool(processes=12) as pool:
        resultDepth = pool.map(
            partial(LoPSComplexity, sets=result["sets"], skipGram=result["skipGram"]),
            filePaths)

    meanDepthPerGame = sum([r[0] for r in resultDepth], [])
    allDepth = sum([r[1] for r in resultDepth], [])
    meanDepthPerSub = np.mean(allDepth)
    data = {
        "allDepth": allDepth,
        "meanDepthPerSub": meanDepthPerSub,
        "meanDepthPerGame": meanDepthPerGame,
        "fileNames": Monkey + ".pkl"
    }
    pd.to_pickle(data, "../MonkeyData/LoPSComplexity/" + date + "/" + Monkey + ".pkl")


if __name__ == '__main__':
    date = "Year3"
    Monkey = "Omega"
    ParsingMain(Monkey, date)
    LoPSC_omplexity_monkey(Monkey, date)
