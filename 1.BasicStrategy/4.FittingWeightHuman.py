...
def combine_context(cutoff_pts, event, data):
    ...
    while i < len(cutoff_pts):
        if need_combine[i] == False:
            ...
            if i != len(cutoff_pts) - 1 and status in accept_combine[i + 1]:  # If the following paragraph can accept the current paragraph
                tail_length = cutoff_pts[i + 1][1] - cutoff_pts[i + 1][0]
                flag = True
            if flag == False:  # If the current paragraph cannot be accepted before or after
                if all_directions_nan(data, cutoff_pts[i]) == False:  # If the current paragraph can exist independently, it exists independently.
                    need_combine[i] = False
                    i += 1
                    continue
                elif i == len(cutoff_pts) - 1:  # If the current paragraph cannot exist independently, it will be forcibly merged into other paragraphs.
                    front_length = cutoff_pts[i - 1][1] - cutoff_pts[i - 1][0]
                elif i == 0:
                    tail_length = cutoff_pts[i + 1][1] - cutoff_pts[i + 1][0]
                elif event[i - 1] == 1:
...
def merge_context(context, cutoff_pts):
    """
    将全为nan的段落的起始点加入cutoff_pts,并保证不被原始分段点切分当前全为nan的段落
    """
    new_cutoff_pts = [0]
    pointer = 0
    end = -1
...
def _combinePhases(bkpt_idx, df, return_flag=False):
    if is_7str == 1:
        agent_list = ["global", "local", "evade_blinky", "evade_clyde", "approach", "energizer", "no_energizer"]
    else:
        agent_list = ["global", "local", "evade_blinky", "evade_clyde", "evade_ghost3", "evade_ghost4", "approach",
                      "energizer"]
    ...
    for each in agent_Q_list:
        ...
        agent_dirs.append(tmp_dirs)
    agent_dirs = np.asarray(agent_dirs)
    same_as_true_dir = np.asarray([(agent_dirs[i] == true_dirs).values for i in range(len(agent_list))])
    ...
    for p in range(len(bkpt_idx)):
        ...
        for i in range(len(agent_list)):
            for j in range(i + 1, len(agent_list)):
                if (i == 2 and j == 3) or (i == 3 and j == 4) or (
                        i == 4 and j == 5):  # ignore two evade agents --ignore 4 evade agents in fMRI version
                    continue
                #If there is nan, skip it
                if np.any(np.isnan(phase_directions[p][i, :])) or np.any(np.isnan(phase_directions[p][j, :])):
                    continue
                if np.mean(phase_directions[p][i, :] == phase_directions[p][j, :]) == 1:  # 把预测方向相同的两个strategy合并？
                    same_num += 1
...
def context_event(x, cutoff_pts, eat_energizer, eat_ghost):
    """
    Time stamp for each paragraph, 0 means all nan, 1 means energizer, 2 means ghost, 3 means ordinary paragraph
    """
    event = []
    is_nan = []
