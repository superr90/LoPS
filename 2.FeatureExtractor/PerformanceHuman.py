...
def reactionTime(filePath):
    """
    Find the time difference (reaction time) between the U-turn point and its previous event (pacman, energizer, ghost)
    :param filePath:
    :return:
    """
    print(filePath)
...
def reactionTimeMain(date):
    fileFolderHuman = "../HumanData/FrameData/" + date + "/"
    fileNamesHuman = os.listdir(fileFolderHuman)
    filePathsHuman = [fileFolderHuman + fileName for fileName in fileNamesHuman]

    with multiprocessing.Pool(processes=12) as pool:
        result = pool.map(partial(reactionTime), filePathsHuman)

    meanReactionTime = [r[0] for r in result]
    reactionTimePerEvent = [r[1] for r in result]
    events = [r[2] for r in result]

    fileNames = ["-".join(fileName.split("-")[:2]) + "-data.pkl" for fileName in fileNamesHuman]
    data = {
    ...
    pd.to_pickle(data, "../HumanData/Performance/" + date + "/" + "reactionTimeHuman.pkl")
...
def rewardMain(date):
    fileFolderHuman = "../HumanData/CorrectedWeightData/" + date + "/"
    fileNamesHuman = os.listdir(fileFolderHuman)
    filePathsHuman = [fileFolderHuman + fileName for fileName in fileNamesHuman]

    reward(filePathsHuman[0])
    with multiprocessing.Pool(processes=12) as pool:
        result = pool.map(partial(reward), filePathsHuman)

    rewardPerGame = [r[0] for r in result]
    tilePerGame = [r[1] for r in result]
    eatGhostNum = [r[2] for r in result]
    fileNames = ["-".join(fileName.split("-")[:2]) + "-data.pkl" for fileName in fileNamesHuman]
    result = {
    ...
    pd.to_pickle(result, "../HumanData/Performance/" + date + "/" + "rewardHuman.pkl")


if __name__ == '__main__':
    date = "session2"
    reactionTimeMain(date)
    rewardMain(date)
