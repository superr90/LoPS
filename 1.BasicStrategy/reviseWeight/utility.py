...
def revise_function(data, context, revise_weight, main_agent, agents, agents_list, agent_num, suffix):
    ...
    data:数据
    context:修改段落
    revise_weight：修改后的weight
    main_agnet:要判断的agent
    """
...
def reviseApproach(data, context, agents, agents_list, agent_num, suffix, strategyToNumber):
    for prev, end in context:
        ...
        x = 0
        # if accuracyApproach == 1 and agent_accuracy[maxIndex] != 1:
        #     continue
...
def reviseMain(path, savePath, strategy_number, agents, agents_list, agent_num, suffix, scared_time):
    ...
    # 确定每个game的结束trial
    game_end = [trial.split("-")[0] for trial in trial_name_list]
    game_not_end = [i for i in range(len(game_end) - 1) if game_end[i + 1] == game_end[i]]
    for t, trial_name in enumerate(trial_name_list):
        ...
        # 修正approach 如果判断为approach且没有吃到鬼则根据概率修改为其他策略
        ...
        # 修正吃到energizer段落的weight
        eat_energizer = np.where(data["eat_energizer"] == True)[0] - 1 + data["level_0"].iloc[0]  # 吃到energizer的前一位置
        eat_energizer_context = list(np.array(data["trial_context"].loc[eat_energizer]))  # 迟到energizer的位置的前一段落
        revise_weight = [0] * len(agents_list)
        ...
        # 如果吃掉一个energizer后有多个approach则合并
        ...
        # 每个trial的最后一个context,如果approach相对较大则改为approach
        # last_beans = data["beans"].iloc[-1]
        # if isinstance(last_beans, float) or len(last_beans) == 0 or t not in game_not_end:
        #     all_trial_record.append(copy.deepcopy(data))
        #     continue
        # #
        # last_context = [data["trial_context"].iloc[-1]]
        # if data["strategy"].iloc[-1] != strategy_number["approach"]:
        #     if last_context[0][1] - last_context[0][0] < 8:
        #         temp_context = sorted(list(set(list(data["trial_context"]))))
        #         if len(temp_context) > 1:
        #             last_context[0] = (temp_context[-2][0], last_context[0][1])
        #     revise_weight = [0] * len(agents_list)
        #     revise_weight[strategy_number["approach"]] = 1
        #     revise_function(data=data, context=last_context, revise_weight=revise_weight,
        #                     main_agent=strategy_number["approach"],
        #                     agents=agents, agents_list=agents_list, agent_num=agent_num, suffix=suffix)
        # data["strategy"] = data[["revise_weight", "is_stay", "is_vague"]].apply(
        #     lambda x: strategyFromWeight(x.revise_weight, x.is_stay, x.is_vague, strategy_number, path, trial_name),
